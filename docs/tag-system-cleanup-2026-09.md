...
2. **Color by idea family** — done on `main` as 6.15.5 (2026-09-08). Eight families + rest in `src/data/tagFamilies.ts`. Chips on `/tag`, rooms, post details, prelude, 404. Not Maslow. Not 221 rainbows. Kitchen rooms (`/tag/cooking`, `/tag/recipes`, `/tag/food`) point to Cookbook.
3. **Fresh usefulness pass** (2026-09-08, uncommitted on `main`) — second walk of all published essays. Dropped hallways (cousin stacks, glossary tags, `metaspace` on pieces that are not about the site). Added missing doors a stranger would click (`puerto-rico` on cooking-for-love, `poems` on poem posts). Recipes already kitchen-clean. Avg ~4 tags/essay. EN/ES twins aligned. 0 cousin stacks. 0 over the 12-tag cap.
