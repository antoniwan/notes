...
3. Validate language/visibility rules:
   - Primary version: `featured: true`
   - Secondary version: `featured: false`
   - Both versions: `published: true` and `draft: false` for toggle visibility
...
- Duplicate listing exposure: ensure only one version in a pair is featured.
