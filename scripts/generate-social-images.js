...
/**
 * Hero art moved under `src/` so Astro can generate responsive derivatives
 * (R20); images referenced from post bodies still live in `public/images`. Both
 * are scanned, and both produce the same logical `/images/…` key, so the
 * manifest, the feeds, and the Open Graph tags are unchanged by the move.
 */
const assetsImagesDir = path.join(__dirname, '../src/assets/images');

/** Path relative to the images namespace, whichever root the file came from. */
function logicalRelative(filePath) {
  const fromAssets = path.relative(assetsImagesDir, filePath);
  if (!fromAssets.startsWith('..') && !path.isAbsolute(fromAssets)) {
    return path.join('images', fromAssets);
  }
...
async function getAvifFiles(rootDir, extensions = ['.avif']) {
  const results = [];
...
async function generateSocialForFile(filePath, previousEntry) {
  const relative = logicalRelative(filePath);
...
export async function writeManifest(
  ...
  // Sorted so the file depends only on its contents. Scanning two image roots
  // made the previous insertion order rewrite 92 unchanged lines.
  const sorted = [...mappings].sort((a, b) => a.original.localeCompare(b.original));
...
async function main() {
  console.log('🔍 Scanning for AVIF images under /public and /src/assets/images ...');
  ...
  const avifFiles = [
    // public/ keeps its AVIF-only rule: it holds book covers, icons and other
    // art that has no business on a social card.
    ...(await getAvifFiles(publicDir)),
    // Everything under src/assets/images is hero art by construction, so the
    // twelve non-AVIF heroes get cards too. Without this their og:image fell
    // back to the original path, which stopped existing when the file moved out
    // of public/ — caught by validate-generated-content.
    ...(await getAvifFiles(assetsImagesDir, ['.avif', '.webp', '.png', '.jpg', '.jpeg'])),
  ];
  ...
  if (avifFiles.length === 0) {
    console.log('No .avif files found in either image root; nothing to generate.');
  ...
  for (const file of avifFiles) {
    const relativeSlash = logicalRelative(file).split(path.sep).join('/');
    const originalWebPath = '/' + relativeSlash;
